...
def add_task():
    ...
    if task:
        task_listBox.insert(tk.END, task) #здесь с помощью константы END подставляем полученное слово в конец списка]
...
def priority_task():
    ...
    if selected_task:
        task_listBox.itemconfig(selected_task, bg="green yellow")
...
priority_button = tk.Button(root, text="Отметить приоритетную задачу", command=priority_task)
priority_button.pack(pady=5)

mark_button = tk.Button(root, text="Отметить выполненную задачу", command=mark_task)
mark_button.pack(pady=5)

alldel_button = tk.Button(root, text="Удалить все задачи из списка", command=delete_all)
alldel_button.pack(pady=5)
